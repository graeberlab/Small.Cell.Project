...
create_log_tcga_files_normal=function() {
  ...
  all.files.short.path=list.files("//10.47.223.100/data2/users/nbalanis/Input_Files_and_Standards/TCGA_TOIL_RECOMPUTE/TOIL_TCGA_norm/temp",pattern="_norm.txt",full.names=F)
  all.names=as.character(sapply(all.files.short.path, function(x) strsplit(x,"_")[[1]][3]))
  for(name in all.names){
    data=read.delim(paste0("//10.47.223.100/data2/users/nbalanis/Input_Files_and_Standards/TCGA_TOIL_RECOMPUTE/TOIL_TCGA_norm/temp/TOIL_TCGA_",name,"_norm.txt"), header =T, stringsAsFactors = F,check.names=F)
    ...
    data=log2_it(data)
    write.table(data,paste0(name,"_rsem_genes_upper_norm_counts_normals_log2.txt"),sep="\t",quote=F,row.names = F)
  }
}