#' MINT Partial Least Squares Discriminant Analysis and predict second dataset
#'
...
MINT.PLSDA_from_file_and_predict_second_dataset<-function (..., file2 , study.names,sample.names, sample.type, y.response, 
  ...
  pls.fit = mint.plsda(X = t.data, Y = y.response, scale = scale, 
                     ncomp = comps,study=my.train.study)
  
  
  write.table(as.data.frame(pls.fit$loadings$X),paste0(output_folder,train_string,  "_MINT.PLSDA_Xloadings.txt"), sep = "\t", row.names = T, 
              quote = F)
  write.table(as.data.frame(pls.fit$variates$X),paste0(output_folder,train_string,  "_MINT.PLSDA_XScores.txt"), sep = "\t", row.names = T, 
              quote = F)
  ...
  prediction.scores <- as.data.frame(test.predict$variates)
  colnames(prediction.scores) <- colnames(x.variates)[-ncol(x.variates)]
  if (varimax == T) {
  ...
  write.table(cbind(Sample = rownames(prediction.scores), (prediction.scores)), 
  ...
  write.table(prediction,paste0(output_folder,test_string,"_projected_onto_",train_string,"_",comps,"_comps_MINT.PLSDA_prediction.txt"),col.names=T,quote=F,sep="\t",row.names=F)
  
  
  
  
  pc.pred <- ggplot(prediction.scores, aes_string(x = comp.x, y = comp.y)) + 
  ...
  pc.pred
  if (plot_both == T) {
    comb = rbind(prediction.scores, x.variates)
